...
class DeterministicPolicyNetwork(BasePolicyNetwork):
    def __init__(self,
        ...
        if action_space is None:
            self.action_scale = nn.Parameter(torch.tensor(1., dtype=torch.float, device=util.device),
                                             requires_grad=False)
            self.action_bias = nn.Parameter(torch.tensor(0., dtype=torch.float, device=util.device),
                                            requires_grad=False)
        elif not isinstance(action_space, Discrete):
            self.action_scale = nn.Parameter(
                torch.tensor((action_space.high - action_space.low) / 2.0, dtype=torch.float, device=util.device),
                requires_grad=False)
...
class GaussianPolicyNetwork(BasePolicyNetwork):
    def __init__(self,
        ...
        self.log_std_min = nn.Parameter(torch.tensor(log_std_min, dtype=torch.float, device=util.device),
                                        requires_grad=False)
        self.log_std_max = nn.Parameter(torch.tensor(log_std_max, dtype=torch.float, device=util.device),
                                        requires_grad=False)
